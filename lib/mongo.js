...
//  console.log('saveText:file=%j', file)
  yield docTable.updateOne({ path: filePath, type: file.type }, file, {upsert: true})
}
...
*/
  ...
  for (var pi = 0; pi < parts.length; pi++) {
    ...
    yield docTable.updateOne({path: part.path, type: part.type, part: pi}, part, {upsert: true})
  ...
  for (var i = 0; i < fileList.length; i++) {
    try {
      ...
      if (filePath.endsWith('.md')) {
        ...
        yield M.saveJson(docTable, mdoJson, filePath, 0, 0)
